...
export default function UpdateProductPage({ productId, onUpdateProduct}: UpdateProductPageProps) {
  ...
  useEffect(() => {
    if (product) {
      ...
      if (product.images && product.images.length > 0) {
        ...
        // Create a dummy FileList to satisfy form validation
        const dummyFile = new File([""], "dummy.jpg", { type: "image/jpeg" });
        const dummyDataTransfer = new DataTransfer();
        dummyDataTransfer.items.add(dummyFile);
        form.setValue('images', dummyDataTransfer.files);
      }
  ...
  const removeImage = (index: number) => {
    ...
    if (isExistingImage) {
      ...
      setUploadedFiles((prev) => prev.filter((_, i) => i !== adjustedIndex));
    }
    
    setPreviewImages((prev) => prev.filter((_, i) => i !== index));

    // If no images left, clear the form field
    if (previewImages.length <= 1) {
      const emptyDataTransfer = new DataTransfer();
      form.setValue('images', emptyDataTransfer.files);
    }
  };
  ...
  const onSubmit = async (values: ProductFormValues) => {
    try {
      ...
      // Only proceed if there are images (either existing or new)
                ...
                <FormItem>
                  ...
                  <FormDescription>
                    {existingImages.length > 0 
                      ? "Current images will be preserved unless removed. Add more images if needed."
